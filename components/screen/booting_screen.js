...
function BootingScreen(props) {
    ...
    return (
        <div style={(props.visible || props.isShutDown ? { zIndex: "100" } : { zIndex: "-20" })} className={(props.visible || props.isShutDown ? " visible opacity-100" : " invisible opacity-0 ") + " absolute duration-500 select-none flex flex-col justify-around items-center top-0 right-0 overflow-hidden m-0 p-0 h-screen w-screen bg-slate-600"}>
            ...
            <p className='flex flex-wrap justify-center text-white'>Anurag Pathak's Ubuntu</p>
            <div className="text-white mb-4">
                <a className="underline" href="https://www.linkedin.com/in/anuragpathak0/" rel="noreferrer noopener" target="_blank">Linkedin</a>
                <span className="font-bold mx-1">|</span>
                <a href="https://github.com/ANURAG-PATHAK" rel="noreferrer noopener" target="_blank" className="underline">Github</a>
            </div>
        </div>
    )
}
